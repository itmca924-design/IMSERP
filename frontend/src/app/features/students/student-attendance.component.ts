...
<div class="page-container">
  ...
  <div *ngIf="selectedStudent" class="attendance-wrapper">
    <mat-card class="period-card mat-elevation-z1">
      ...
      <button mat-raised-button color="primary" (click)="openNewRecord()"><mat-icon>add_task</mat-icon> Mark Attendance</button>
    </mat-card>
    ...
    <mat-card class="records-card mat-elevation-z1" *ngIf="records.length">
      ...
      <div class="record-row" *ngFor="let record of records"><div><strong>{{ record.attendanceDate | date:'dd MMM yyyy' }}</strong><small>{{ record.attendanceDate | date:'EEEE' }}</small></div><span class="status" [ngClass]="record.status.toLowerCase()">{{ record.status === 'HalfDay' ? 'Half Day' : record.status }}</span><span class="remarks">{{ record.remarks || '—' }}</span><button mat-icon-button color="primary" (click)="editRecord(record)" [disabled]="isPublicHolidayOrSunday(record.attendanceDate) && !canEditPublicHolidayOrSunday" matTooltip="Edit record"><mat-icon>edit</mat-icon></button><button mat-icon-button color="warn" (click)="deleteRecord(record.id)" [disabled]="isPublicHolidayOrSunday(record.attendanceDate) && !canEditPublicHolidayOrSunday" matTooltip="Delete record"><mat-icon>delete_outline</mat-icon></button></div>
    </mat-card>
  </div>
</div>
  `,
